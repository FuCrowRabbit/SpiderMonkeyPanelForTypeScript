...
const Point2Pixel = (pt: number, dpi: number): number => {
    return (pt * dpi / 72);
};
...
const toRGB = (col: number): number[] => { // returns an array like [192, 0, 0=> ]
    const a = col - 0xFF000000;
    return [a >> 16, a >> 8 & 0xFF, a & 0xFF];
};

const getAlpha = (colour: number): number => {
    return ((colour >> 24) & 0xff);
};

const getRed = (colour: number): number => {
    return ((colour >> 16) & 0xff);
};

const getGreen = (colour: number): number => {
    return ((colour >> 8) & 0xff);
};

const getBlue = (colour: number): number => {
    return (colour & 0xff);
};

const setAlpha = (colour: number, a: number): number => {
    return ((colour & 0x00ffffff) | (a << 24));
};

const setRed = (colour: number, r: number): number => {
    return ((colour & 0xff00ffff) | (r << 16));
};

const setGreen = (colour: number, g: number): number => {
    return ((colour & 0xffff00ff) | (g << 8));
};

const setBlue = (colour: number, b: number): number => {
    return ((colour & 0xffffff00) | b);
};

// Helper function for DrawString() and MeasureString()
// args: h_align, v_align, trimming, flags
const StringFormat = function (): number {
