...
function Point2Pixel(pt: number, dpi: number): number {
    return (pt * dpi / 72);
}
...
function toRGB(col: number): number[] { // returns an array like [192, 0,  ]
    const a = col - 0xFF000000;
    return [a >> 16, a >> 8 & 0xFF, a & 0xFF];
}

function getAlpha(colour: number): number {
    return ((colour >> 24) & 0xff);
}

function getRed(colour: number): number {
    return ((colour >> 16) & 0xff);
}

function getGreen(colour: number): number {
    return ((colour >> 8) & 0xff);
}

function getBlue(colour: number): number {
    return (colour & 0xff);
}

function setAlpha(colour: number, a: number): number {
    return ((colour & 0x00ffffff) | (a << 24));
}

function setRed(colour: number, r: number): number {
    return ((colour & 0xff00ffff) | (r << 16));
}

function setGreen(colour: number, g: number): number {
    return ((colour & 0xffff00ff) | (g << 8));
}

function setBlue(colour: number, b: number): number {
